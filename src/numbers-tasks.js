...
function getCircleCircumference(radius) {
  const resCircle = radius * 2 * Math.PI;
  return resCircle;
}
...
function getAverage(value1, value2) {
  if (value1 === Number.MAX_VALUE) {
    const resAv = value1 / 2 + value2 / 2;
    return resAv;
  }
  const resAv = (value1 + value2) / 2;
  return resAv;
}
...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const resAv = Math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2);
  return resAv;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const sumRes = x1 * x2 + y1 * y2;
  const res1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const res2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosAng = sumRes / (res1 * res2);
  const res = Math.acos(cosAng);
  return res;
}
...
function getParallelepipedDiagonal(a, b, c) {
  const res1 = Math.sqrt(a ** 2 + b ** 2 + c ** 2);
  return res1;
}
...
function roundToPowerOfTen(num, pow) {
  ...
  const resRound = num / 10 ** pow;
  const res = Math.round(resRound) * 10 ** pow;
  return res;
}
...
function getSumOfDigits(num) {
  ...
  const numNum = numArr.map(Number);
  let sum = 0;
  for (let i = 0; i < numNum.length; i += 1) {
    sum += numNum[i];
  }
  return sum;
}
...
function getCountOfOddNumbers(number) {
  const numRes = Math.abs(number);
  return Math.floor((numRes + 1) / 2);
}
